...
`version` field of `.claude-plugin/plugin.json`; the newest published version is that
...
MindArt from the refreshed copy. Start a new session afterwards so the updated
